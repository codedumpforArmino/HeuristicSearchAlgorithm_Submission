...
namespace ACT4
{
    public partial class Form1 : Form
    {
        ...
        private ArrayList getBestMoves(int[,] heuristicTable)
        {
            ...
            for (int i = 0; i < n; i++)
            {
                for (int j = 0; j < n; j++)
                {
                    ...
                    } else if (bestHeuristicValue == heuristicTable[i,j] || new Random().NextDouble() <= calcTemperature(heuristicTable[i, j]))  //if same weights, take chance on temperature 
                    {
                        if (currentState.Y[i] != j)
                            bestMoves.Add(new Point(i, j));
                    }
        ...
        public double calcTemperature(int hValue)
        {
